"""
 @file: sender_tahoe.py
 @brief: TCP Tahoe congestion control protocol implementation
        ...
        while True:
            while len(packets) < cwnd and (curr_state.value < State.SENDING_EOF.value):
                ...
                if chunk:
                    # add payload
                    packet += chunk
                    packets.append((seq_id_global, None, packet, False))   # Append seq_id, time_sent, packet (for easy retransmission), and in_transit flag
            ...
            try:
                ...
                if ack_id == last_ack:
                    ...
                    timeout = 35 * estimated_RTT
            except socket.timeout:
                if curr_state == State.SENDING_EOF:
                    end_time = time.time()
                    break
                timeout_reset()  # Socket timeout
                continue
